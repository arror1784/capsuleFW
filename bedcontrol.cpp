...
//    qDebug() << "bedControl - receiveFromSerialPort bed Char : " << bedChar << " bedState :" << bedState;
    if(bedChar == this->bedChar){
        ...
        case PRINT_MOVE_INIT:
            ...
            break;
        case PRINT_MOVE_WAIT:
             emit sendToPrintScheduler(bedChar,PRINT_RESUME);
            break;
        case PRINT_MOVE_PAUSE_WAIT:
             bedState = PRINT_MOVE_WAIT;
             break;
...
void BedControl::moveDownCommand(){
    char buffer[50] = {0};
    sprintf(buffer,"G01 %c%lf",bedChar,-(ZHopHeight - LayerHeight));
    emit sendCommand(buffer);
...
void BedControl::moveUpCommandMax(){
    ...
    emit sendCommand("G02 A-15");
    currentPosition = -15;
    current_function = nullptr;
}
void BedControl::moveDownCommandMin(){
    char buffer[50] = {0};
    sprintf(buffer,"G01 %c%lf",bedChar,-(maxHeight - LayerHeight));
    emit sendCommand(buffer);
...
void BedControl::setLayerHeightTime(const double value){
