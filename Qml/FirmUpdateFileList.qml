...
Item {
    ...
    Text {
        id: selectText
        text: qsTr("Select a file to update")
...
//                        parentDirText.text = fileSelectList.indexAt(0).filename
    ...
    Rectangle{
        ...
        MouseArea{
            ...
            onClicked: {
                if(selectedFileName !== ""){
                    console.log(selectedFileName,currentPath)
                    scheduler.receiveFromQmlFirmUpdate(currentPath+"/"+selectedFileName)
    ...
    Connections{
        ...
        target: scheduler
        onSendToFirmwareUpdateFinish:{
            firmwareUpdatePopup.setBTNEnabled(true)
        }
        onSendToFirmwareUpdateError:{
            firmwareUpdatePopup.setBTNEnabled(true)
            firmwareUpdatePopup.setTest("firmupdate fail")
        }
        onSendToFirmwareUpdateProgrese:{
            firmwareUpdatePopup.setProgress(progress)
        }
    }
}
